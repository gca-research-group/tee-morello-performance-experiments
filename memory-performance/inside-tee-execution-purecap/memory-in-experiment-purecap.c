 ...
 *              :
 *              : Compile and execute as follows to create and use
 *              : library-based compartments.
 * Compile      :
 * Capabilities : clang-morello -march=morello+c64 -mabi=purecap -g -o memory-in-experiment memory-in-experiment.c -L. -lm 
 *              :
...
int main() {
    ...
    */
    ...
    printf(" \nmemory allocate, write, read, free in execution now (using cheri compartments) ...\n");
